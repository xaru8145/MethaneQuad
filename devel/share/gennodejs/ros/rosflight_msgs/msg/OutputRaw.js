...
class OutputRaw {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    if (obj.values.length !== 16) {
      throw new Error('Unable to serialize array field values - length must be 16')
    }
    // Serialize message field [values]
    bufferOffset = _arraySerializer.float32(obj.values, buffer, bufferOffset, 16);
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    data.values = _arrayDeserializer.float32(buffer, bufferOffset, 16)
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.values !== undefined) {
      ...
      resolved.values = new Array(16).fill(0)
    }
